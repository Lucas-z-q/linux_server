...
|   |-- roadmap.md
|   |-- benchmark.md
|   |-- master_bugfix_report.md
|   |-- real_backend_pool_benchmark_report.md
|   |-- real_backend_stress_report.md
|   `-- superpowers/
|       |-- plans/
|       `-- specs/
|-- include/
...
`scripts/` 下的脚本用于本地开发环境探测、运行与测试执行（含 WSL 辅助脚本、缓存与 Redis 测试运行脚本），当前未纳入版本控制。
...
- 缓存与 Redis：`redis_*_test`、`chat_service_redis_test`、`chat_service_cache_test`、`cross_server_push_router_test`
