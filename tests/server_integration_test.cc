...
void TestHeartbeatRouteOverTcp() {
    const nlohmann::json resp = SendAndReceive(R"({"msg_type":"heartbeat","seq":1,"token":"","data":{}})");
...
void TestUnknownRouteOverTcp() {
    const nlohmann::json resp = SendAndReceive(R"({"msg_type":"chat","seq":2,"token":"","data":{}})");

    ExpectCommonEnvelope(resp, "chat_resp", 2, chat::ErrorCode::UNKNOWN_MESSAGE_TYPE);
}
...
void TestLoginValidationOverTcp() {
    ...
    ExpectCommonEnvelope(resp, "login_resp", 3, chat::ErrorCode::INVALID_PARAM);
    assert(resp["message"].get<std::string>().find("password") != std::string::npos);
}

void TestLoginDbQueryFailureOverTcp() {
    const nlohmann::json resp =
        SendAndReceive(R"({"msg_type":"login","seq":4,"token":"","data":{"username":"alice","password":"123456"}})");

    ExpectCommonEnvelope(resp, "login_resp", 4, chat::ErrorCode::DB_QUERY_FAILED);
...
void TestRegisterDbQueryFailureOverTcp() {
    ...
    ExpectCommonEnvelope(resp, "register_resp", 5, chat::ErrorCode::DB_QUERY_FAILED);
    assert(resp["message"].get<std::string>() == "query user failed");
}

void TestInvalidJsonOverTcp() {
    const nlohmann::json resp = SendAndReceive(R"({"msg_type":"login","seq":8,"data":)");
...
void TestWhoAmIAndLogoutRequireLoginOnSameConnection() {
    ...
    ExpectCommonEnvelope(whoami_resp, "whoami_resp", 6, chat::ErrorCode::USER_NOT_FOUND);
    ...
    ExpectCommonEnvelope(logout_resp, "logout_resp", 7, chat::ErrorCode::USER_NOT_FOUND);
